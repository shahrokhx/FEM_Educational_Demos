...
%__________________________________________________________________________
%
% Bar element shape function (3-node (second-order) bar element)
...
for k = 1 : nElemNode
    idx = k + 2;
    if nElemNode == 2
        axes(idx) = subplot(2,4,[4+2*k-1,4+2*k]);
    elseif nElemNode == 3 
...
dCoord = coordinates(:,end) - coordinates(:,1);
L      = norm(dCoord);
coordinates = [0 L];
c = dCoord(2)/L;
s = dCoord(1)/L;
...
if abs(xi)<=1 && abs(eta)<=1
    ...
    N = shapefunctions(1,nElemNode,xi);
    ...
    elseif nElemNode == 3
        ...
        N1  = shapefunctions(1,nElemNode-1,xi);
        tmp = N1(1); N1(1) = N1(2); N1(2) = tmp;
        xyMap = coordinates * N1;
        xyMap = (T * [0 xyMap]') + node1;
    end
